...
export const createProductOnServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    mutateServerProducts((ps) => [product, ...ps]);
...
export const updateProductOnServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    if (!getServerProducts().some((p) => p.id === data.id)) return { error: "Product not found" };
    mutateServerProducts((ps) => ps.map((p) => (p.id === data.id ? { ...p, ...data.patch } : p)));
...
export const removeProductOnServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    mutateServerProducts((ps) => ps.filter((p) => p.id !== data.id));
    return { ok: true as const };
  });
...
export const setProductCostOnServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    mutateServerProducts((ps) => ps.map((p) => (p.id === data.id ? { ...p, cost: data.cost } : p)));
    return { ok: true as const };
  });
...
export const increaseStockOnServer = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    adjustStock(data.id, data.qty);
    const stock = getServerProducts().find((p) => p.id === data.id)?.stock ?? 0;
